...
def image_create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            # Shakldagi # ta maʼlumotlar haqiqiy
            cd = form.cleaned_data
            new_image = form.save(commit=False)
            # elementga joriy foydalanuvchini tayinlash
            new_image.user = request.user
            new_image.save()
            create_action(request.user, 'bookmarked image', new_image)
            messages.success(request, 'Image added successfully')
            # batafsil ko'rinishga yo'naltirish
            # yangi yaratilgan element haqida ma'lumot
            return redirect(new_image.get_absolute_url())
    else:
        # shaklni ma'lumotlar bilan bog'lash,
        # GET usuli yordamida xatcho'p tomonidan taqdim etilgan
